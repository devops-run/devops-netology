...
```
### Вывод:
При запуске контейнера с монтированием одного каталога из хост машины (опция  -v, --volume Bind mount a volume) все контейнеры имеют полный доступ к файловой сисиеме хост машины (чтение/запись) в рамках подмонтированного при запуске каталога.
